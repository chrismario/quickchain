...
func (h *Header) Hash() common.Hash {
	...
	if h.MixDigest == BFTDigest {
		...
		if bftHeader := BFTFilteredHeader(h, true); bftHeader != nil {
			return rlpHash(bftHeader)
		}
	}
	return rlpHash(h)
}
...
type Block struct {
	...
	// These fields are used by package qct to track
...
// [deprecated by let/63]
// StorageBlock defines the RLP encoding of a Block stored in the
// state database. The StorageBlock encoding contains fields that
// would otherwise need to be recomputed.
type StorageBlock Block

// "external" block encoding. used for let protocol, etc.
