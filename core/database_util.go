...
var (
	...
	configPrefix   = []byte("quickchain-config-") // config prefix for the db
...
func GetBlock(db DatabaseReader, hash common.Hash, number uint64) *types.Block {
	...
	block.DposContext = getDposContextTrie(db.(qctdb.Database), header)
	return block
}

func getDposContextTrie(db qctdb.Database, header *types.Header) *types.DposContext {
...
func WriteCanonicalHash(db qctdb.Putter, hash common.Hash, number uint64) error {
...
func WriteHeadHeaderHash(db qctdb.Putter, hash common.Hash) error {
...
func WriteHeadBlockHash(db qctdb.Putter, hash common.Hash) error {
...
func WriteHeadFastBlockHash(db qctdb.Putter, hash common.Hash) error {
...
func WriteTrieSyncProgress(db qctdb.Putter, count uint64) error {
...
func WriteHeader(db qctdb.Putter, header *types.Header) error {
...
func WriteBody(db qctdb.Putter, hash common.Hash, number uint64, body *types.Body) error {
...
func WriteBodyRLP(db qctdb.Putter, hash common.Hash, number uint64, rlp rlp.RawValue) error {
...
func WriteTd(db qctdb.Putter, hash common.Hash, number uint64, td *big.Int) error {
...
func WriteBlock(db qctdb.Putter, block *types.Block) error {
...
func WriteBlockReceipts(db qctdb.Putter, hash common.Hash, number uint64, receipts types.Receipts) error {
...
func WriteTxLookupEntries(db qctdb.Putter, block *types.Block) error {
...
func WriteBloomBits(db qctdb.Putter, bit uint, section uint64, head common.Hash, bits []byte) {
	key := append(append(bloomBitsPrefix, make([]byte, 10)...), head.Bytes()...)
...
func PreimageTable(db qctdb.Database) qctdb.Database {
	return qctdb.NewTable(db, preimagePrefix)
}

// WritePreimages writes the provided set of preimages to the database. `number` is the
// current block number, and is used for debug messages only.
func WritePreimages(db qctdb.Database, number uint64, preimages map[common.Hash][]byte) error {
...
func WriteBlockChainVersion(db qctdb.Putter, vsn int) {
	enc, _ := rlp.EncodeToBytes(uint(vsn))
	db.Put([]byte("BlockchainVersion"), enc)
}

// WriteChainConfig writes the chain config settings to the database.
func WriteChainConfig(db qctdb.Putter, hash common.Hash, cfg *params.ChainConfig) error {
